...
(function  ($) {
	$.guidance = function  (json,fn) {
		...
		json.style.bgImg = json.style.bgImg || "url(img/专属客服.png)";//背景图片
		// json.style.point1 = json.style.point1 || 'url(img/专属客服.png)';
		// json.style.point2 = json.style.point2 || 'url(img/专属客服.png)';
		var index = 0;
		if (json.obj.length>0){
			...
			if ($.isArray(json.obj)){
				...
				$('#nextStep').bind('click',function  () {
						index++;
						if(index==1){
							$('#warnData').css('background','url(img/2.jpg)');
						}else if(index==2){
							$('#warnData').css('background','url(img/8.jpg)');
						}
